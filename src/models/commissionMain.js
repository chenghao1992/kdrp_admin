import { exportMechanism, queryListMain } from '../services/commissionMain'
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/report/report_agency') {
  ...
  effects: {
    ...
    *search ({ payload }, { put }) {
    ...
    /** searchInit ({payload}, {call, put, select}) {
      const reloadFlag=yield  select(state => state.commissionMain.isReload)
      if(reloadFlag){
    ...
    *queryOrgTree ({ payload }, { call, put, select }) {
      const data = yield call(queryOrgTree, { is_bottom: false })
      if (data && data.status === 'success') {
        yield put({ type: 'queryOrgTreeSuccess', payload: data.data })

        const queryData = yield select(state => state.commissionMain.queryParam)
    ...
    *queryList ({ payload }, { call, put, select }) {
      ...
      const queryData = yield select(state => state.commissionMain.queryParam)
      const data = yield call(queryListMain, { ...queryData, ...payload })
    ...
    *btnExport ({ payload }, { call }) {
      const data = yield call(exportMechanism, payload)
      if (data && data.status === 'success') {
