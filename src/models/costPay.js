import { batchPay, queryListPay } from '../services/costPay'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/cost/costPay') {
  ...
  effects: {
    ...
    *queryList ({ payload }, { call, put, select }) {
      ...
      const queryData = yield select(state => state.costPay.queryParam)
      const data = yield call(queryListPay, { ...queryData, ...payload })

      yield put({ type: 'setReset', payload: false })

      if (data && data.status === 'success') {
        yield put({
          type: 'queryListSuccess', payload: {
            list: data.data,
    ...
    *batchAction ({
      ...
      yield put({ type: 'showLoading' })
      const data = yield call(batchPay, parse(payload.param))
      if (data && data.status === 'success') {
        message.success('操作成功！3秒后将自动刷新列表', 2)
        yield put({
          type: 'queryList',
        })

        setTimeout(() => {
          payload.onHandChange()
          console.log(1)
        }, 3000)
      } else {
