 ...
 */
import { investReportAgency, treeLimit2, agencyExport } from '../services/reportStaff'
import { message } from 'antd'


export default {
  namespace: 'reportStaffOrganization',
  state: {
    tree: [],
    list: [],
    searchCode: {},
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/report/report_staff_organization') {
  ...
  effects: {
    ...
    *investList ({ payload }, { call, put }) {
      const data = yield call(investReportAgency, payload)
      if (data && data.status === 'success') {
        yield put({
          type: 'investLists',
          payload: data.data,
    ...
    *onExport ({ payload }, { call }) {
      const data = yield call(agencyExport, payload)
      if (data && data.status === 'success') {
