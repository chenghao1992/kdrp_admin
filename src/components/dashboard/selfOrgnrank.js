...
function SelfOrgnRank ({ data, header, scrollRank, highterStatus }) {
  let totalvalue = 1
  const colum = [
    {
      title: '',
    ...
    <div>
      {highterStatus === 3 ?
        <div className={styles.orgnrank3}>
          <Table size="small" pagination={false} showHeader columns={columns} rowKey={(record, key) => key} dataSource={data} scroll={{ y: scrollRank }} />
        </div> :
...
SelfOrgnRank.propTypes = {
  data: PropTypes.array,
  header: PropTypes.array,
  changeHeightOfNumCard: PropTypes.bool,
  scrollRank: PropTypes.number,
  highterStatus: PropTypes.string,
}

export default SelfOrgnRank
